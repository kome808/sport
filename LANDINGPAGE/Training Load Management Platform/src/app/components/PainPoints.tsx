...
export function PainPoints() {
  const painPoints = [
    ...
    },
    {
      ...
      icon: Heart,
      title: "家長的不安",
      description: "孩子每天高強度訓練，卺缺乏一個能理解身體負荷與訓練狀況的管道。",
    },
    {
      number: "03",
      icon: Settings,
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl lg:text-6xl font-black text-white mb-6 tracking-tight">
            基層運動隊伍<br />
            <span className="text-[#c1ff00]">最常遇到的</span>三個問題
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {painPoints.map((point, index) => {
            ...
            return (
              ...
              >
                {/* Number Badge */}
                <div className="absolute -top-4 -right-4 w-16 h-16 bg-[#c1ff00] flex items-center justify-center">
                  <span className="text-black font-black text-2xl">{point.number}</span>
                </div>

                {/* Icon */}
                <div className="w-16 h-16 bg-white/5 group-hover:bg-[#c1ff00]/10 flex items-center justify-center mb-6 transition-colors">
                  <Icon className="w-8 h-8 text-[#c1ff00]" />
                </div>
                ...
                {/* Accent Line */}
                <div className="absolute bottom-0 left-0 w-0 h-1 bg-[#c1ff00] group-hover:w-full transition-all duration-300"></div>
              </div>
            );
          })}
        </div>
