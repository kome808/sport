-- Migration: Fix Search Path (Part 2 - Logic & Demo)
-- Date: 2026-01-31
-- Description: Explicitly set search_path to 'sport, public, extensions, pg_catalog' for business logic and demo functions.
...
-- 4. sport.calculate_rhr_baseline (Table) - Using latest logic (7 days, +5/+10 thresholds)
...
-- 6. sport.calculate_weekly_load_change (Table) - Using latest logic (min 3 days)
...
-- 7. sport.get_player_fatigue_metrics (JSONB) - Using latest logic
...
-- 8. sport.calculate_acwr_decoupled (Table) - From 023
...
BEGIN
  ...
  IF v_team_id IS NULL THEN RAISE EXCEPTION '找不到球隊 slug: %', p_team_slug; END IF;
    ...
    FOR i IN 0..(p_days_to_check - 1) LOOP
      ...
      IF NOT EXISTS (SELECT 1 FROM sport.daily_records WHERE player_id = v_player_id AND record_date = v_date) THEN
        ...
        IF EXTRACT(ISODOW FROM v_date) IN (6, 7) THEN
           v_base_minutes := 40; 
           v_base_rpe := 8 + floor(random() * 3)::INT; 
        ELSE
           IF v_role = 'high_load' THEN v_base_minutes := 120 + floor(random() * 30)::INT; v_base_rpe := 6 + floor(random() * 3)::INT;
           ELSIF v_role = 'fatigue_prone' THEN v_base_minutes := 90 + floor(random() * 20)::INT; v_base_rpe := 7 + floor(random() * 3)::INT; 
           ELSE v_base_minutes := 100 + floor(random() * 20)::INT; v_base_rpe := 5 + floor(random() * 3)::INT;
           END IF;
        END IF;

        IF EXTRACT(ISODOW FROM v_date) = 7 THEN v_base_minutes := 0; v_base_rpe := 0; END IF;

        IF v_base_minutes > 0 THEN
            INSERT INTO sport.daily_records (
              player_id, record_date, training_minutes, srpe_score, rhr_bpm,
              sleep_quality, fatigue_level, mood, stress_level, muscle_soreness, feedback
            ) VALUES (
              v_player_id, v_date, v_base_minutes, v_base_rpe, 60 + floor(random() * 10)::INT,
              3 + floor(random() * 3)::INT, 2 + floor(random() * 2)::INT, 4, 4, 
              CASE WHEN v_role = 'injury_risk' AND floor(random()*10) > 7 THEN 4 ELSE 2 END,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Part 2 Migrations Applied (Fatigue Logic & Demo)';
END $$;
