-- ================================================
-- 修復演示帳號 V3 (Fix Demo Account - Identity Columns)
-- 日期: 2026-01-25
-- 說明：修正 auth.identities 缺少 provider_id 的問題
-- ================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM auth.identities WHERE user_id = v_demo_id) THEN
        INSERT INTO auth.identities (
            ...
            'email',
            v_demo_id::text, -- 對於 email provider，通常使用 user_id 作為 provider_id
            now(),
            now(),
            now(),
            v_demo_email
        );
        RAISE NOTICE '已建立 Identity for % (with provider_id)', v_demo_email;
    ELSE
        -- 確保 Identity 資料正確 (避免 provider_id 是 null 的舊資料卡住)
    ...
    RAISE NOTICE 'Demo account setup V3 complete for %', v_demo_email;
